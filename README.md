...
- 🛡️ **Local-First Architecture**: No cloud servers, external databases, or third-party APIs (except for Google Map tiles). All profile settings, emergency contacts, active trip states, and alert logs are stored safely on-device in a Room database.
...
- 🗺️ **Interactive Google Maps View**: Displays live-updating markers for the start location, destination, and current position. Renders a polyline route connecting the points and automatically centers the camera as the traveler moves.
...
- ⚙️ **Route Simulation Mode**: Built-in interpolation simulator that generates a 100-step path between start and finish points. Travelers can adjust simulation speed (1x up to 100x speed) in real time using a custom slider.
